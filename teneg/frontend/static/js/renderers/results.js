...
import { initDrapeComparison } from './drape-comparison.js';
...
export function displayResults(data) {
    ...
    /* Drape comparison in colors view */
    const src = dom.previewImage?.src || '';
    const goodColors = (recommendations.clothing?.tops || []).map(c =>
        typeof c === 'string' ? { hex: c, name: c } : c);
    const avoidColors = (recommendations.avoid || []).map(c =>
        typeof c === 'string' ? { hex: c, name: c } : c);
    try { initDrapeComparison(src, goodColors, avoidColors); } catch { /* non-critical */ }

    /* Re-apply i18n so newly visible data-i18n elements get their translations. */
    applyLanguage(state.lang);
    /* Sidebar SPA: navigate to results view and populate overview/dashboard */
    if (typeof window._afterDisplayResults === 'function') {
        window._afterDisplayResults(data);
    } else {
